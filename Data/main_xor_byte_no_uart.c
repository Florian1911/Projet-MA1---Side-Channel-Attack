...
/**
  ******************************************************************************
  * @file           : main_xor_byte_no_uart.c
  * @brief          : Etape intermediaire CPA: fuite d'un byte XOR cle.
  *                   v = PT[TARGET_BYTE] xor KEY[TARGET_BYTE]
  *                   puis transitions 0 <-> v sous trigger.
  ******************************************************************************
  */
...
int main(void)
{
  ...
  for (uint32_t i = 0; i < N_PLAINTEXTS; i++) {
    ...
    for (uint32_t r = 0; r < LEAK_REPEATS; r++) {
      const uint8_t v = (uint8_t)(p ^ KEY_128[TARGET_BYTE]);
      leak_reg = v;
      __NOP();
      leak_reg = 0u;
      __NOP();
