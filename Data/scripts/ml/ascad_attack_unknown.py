#!/usr/bin/env python3
import argparse
import json
from pathlib import Path

import numpy as np

try:
    import tensorflow as tf
except Exception as exc:  # pragma: no cover
    raise SystemExit(
        "TensorFlow is required for ASCAD attack.\n"
        "Install with: python -m pip install tensorflow-cpu\n"
        f"Import error: {exc}"
    )

AES_SBOX = np.array([
    
], dtype=np.uint8)


def center_and_detrend(traces: np.ndarray) -> np.ndarray:
    t = traces.astype(np.float64, copy=False)
    t = t - t.mean(axis=1, keepdims=True)
    x = np.linspace(-1.0, 1.0, t.shape[1], dtype=np.float64)
    x2 = float(np.dot(x, x))
    slopes = (t @ x) / x2
    t = t - np.outer(slopes, x)
    return t.astype(np.float32)


def crop_window(traces: np.ndarray, center: int, window: int) -> np.ndarray:
    half = window // 2
    w0 = max(0, center - half)
    w1 = min(traces.shape[1], center + half)
    return traces[:, w0:w1]


def key_scores_for_shift(logp: np.ndarray, pbyte: np.ndarray, shift: int) -> np.ndarray:
    n = logp.shape[0]
    if shift >= 0:
        n_eff = n - shift
        lp = logp[:n_eff]
        pb = pbyte[shift:]
    else:
        s = -shift
        n_eff = n - s
        lp = logp[s:]
        pb = pbyte[:n_eff]
    keys = np.arange(256, dtype=np.uint8)
    sbox_map = AES_SBOX[np.bitwise_xor(pb[:, None], keys[None, :])]
    return np.sum(lp[np.arange(n_eff)[:, None], sbox_map], axis=0)


def parse_true_key(s: str | None) -> np.ndarray | None:
    if not s:
        return None
    t = s.strip().replace(" ", "").replace(":", "").replace(",", "").upper()
    if len(t) != 32:
        raise ValueError("--true-key-hex must contain 32 hex chars")
    return np.array([int(t[i:i + 2], 16) for i in range(0, 32, 2)], dtype=np.uint8)


def main() -> None:
    ap = argparse.ArgumentParser(description="Attack unknown key byte using trained ASCAD CNN")
    ap.add_argument("--model", required=True, help=".keras model path")
    ap.add_argument("--stats-npz", required=True, help=".npz containing mu/sigma")
    ap.add_argument("--attack-npz", required=True)
    ap.add_argument("--byte", type=int, required=True)
    ap.add_argument("--center", type=int, default=-1)
    ap.add_argument("--window", type=int, default=700)
    ap.add_argument("--preproc", choices=["none", "center_detrend"], default="center_detrend")
    ap.add_argument("--max-shift", type=int, default=80)
    ap.add_argument("--batch-size", type=int, default=1024)
    ap.add_argument("--true-key-hex", default="")
    ap.add_argument("--out", default="ascad_recover_byte.json")
    args = ap.parse_args()

    d = np.load(args.attack_npz)
    traces = d["traces"].astype(np.float32)
    plains = d["plaintexts"].astype(np.uint8)
    pbyte = plains[:, args.byte]

    if args.center >= 0:
        traces = crop_window(traces, args.center, args.window)
    if args.preproc == "center_detrend":
        traces = center_and_detrend(traces)

    stats = np.load(args.stats_npz)
    mu = stats["mu"].astype(np.float32)
    sigma = stats["sigma"].astype(np.float32)
    x = ((traces - mu) / (sigma + 1e-6)).astype(np.float32)[..., None]

    model = tf.keras.models.load_model(args.model, compile=False)
    probs = model.predict(x, batch_size=args.batch_size, verbose=0).astype(np.float64)
    probs = np.clip(probs, 1e-12, 1.0)
    logp = np.log(probs)

    best = None
    for s in range(-args.max_shift, args.max_shift + 1):
        scores = key_scores_for_shift(logp, pbyte, s)
        order = np.argsort(scores)[::-1]
        row = {
            "shift": int(s),
            "top1": int(order[0]),
            "top2": int(order[1]),
            "margin": float(scores[order[0]] - scores[order[1]]),
            "scores": scores,
        }
        if best is None or row["margin"] > best["margin"]:
            best = row

    order = np.argsort(best["scores"])[::-1]
    top5 = [{"rank": i + 1, "key": int(k), "score": float(best["scores"][k])} for i, k in enumerate(order[:5])]

    out = {
        "model": args.model,
        "stats_npz": args.stats_npz,
        "attack_npz": args.attack_npz,
        "byte": int(args.byte),
        "best_shift": int(best["shift"]),
        "recovered_key_byte": int(best["top1"]),
        "margin": float(best["margin"]),
        "top5": top5,
    }

    true_key = parse_true_key(args.true_key_hex)
    if true_key is not None:
        true_b = int(true_key[args.byte])
        rank = int(np.where(order == true_b)[0][0]) + 1
        out["true_key_byte"] = true_b
        out["true_rank"] = rank

    with open(args.out, "w") as f:
        json.dump(out, f, indent=2)

    print(f"Recovered byte[{args.byte}] = 0x{best['top1']:02X}")
    print(f"best_shift={best['shift']} margin={best['margin']:.6f}")
    for r in top5:
        print(f"  #{r['rank']}: 0x{r['key']:02X} score={r['score']:.4f}")
    if true_key is not None:
        print(f"true=0x{int(true_key[args.byte]):02X} rank={out['true_rank']}")
    print("saved:", args.out)


if __name__ == "__main__":
    main()
