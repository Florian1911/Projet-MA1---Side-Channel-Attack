...
/**
  ******************************************************************************
  * @file           : main_aes_byte_no_uart.c
  * @brief          : Cible intermediaire CPA no-UART : fuite d'un seul byte AES
  *                   v = SBOX(PT[TARGET_BYTE] xor KEY[TARGET_BYTE])
  *                   repetee sous trigger pour valider byte par byte.
  ******************************************************************************
  */
...
int main(void)
{
  ...
  for (uint32_t i = 0; i < N_PLAINTEXTS; i++) {
    ...
    const uint8_t v = naive_sbox[(uint8_t)(p ^ KEY_128[TARGET_BYTE])];
