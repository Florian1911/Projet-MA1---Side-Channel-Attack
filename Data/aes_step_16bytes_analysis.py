#!/usr/bin/env python3
import argparse
import json
import numpy as np
import matplotlib.pyplot as plt

SBOX = np.array([
    
], dtype=np.uint8)
HW = np.array([bin(i).count('1') for i in range(256)], dtype=np.float64)


def preprocess(tr):
    t = tr.astype(np.float64, copy=False)
    t = t - t.mean(axis=1, keepdims=True)
    x = np.linspace(-1.0, 1.0, t.shape[1], dtype=np.float64)
    t = t - np.outer((t @ x) / np.dot(x, x), x)
    return t


def cpa_corr_curve(tr, pbyte, key):
    tc = tr - tr.mean(axis=0, keepdims=True)
    tstd = tr.std(axis=0, ddof=1) + 1e-15
    h = HW[SBOX[np.bitwise_xor(pbyte, key)]]
    hc = h - h.mean()
    hstd = h.std(ddof=1) + 1e-15
    return (hc @ tc) / ((tr.shape[0] - 1) * hstd * tstd)


def rank_and_scores_at_poi(y, pbyte, true_key):
    yc = y - y.mean()
    ystd = y.std(ddof=1) + 1e-15
    scores = np.zeros(256, dtype=np.float64)
    for k in range(256):
        h = HW[SBOX[np.bitwise_xor(pbyte, k)]]
        hc = h - h.mean()
        hstd = h.std(ddof=1) + 1e-15
        scores[k] = abs((hc @ yc) / ((len(y) - 1) * hstd * ystd))
    order = np.argsort(scores)[::-1]
    rank = int(np.where(order == true_key)[0][0]) + 1
    top = int(order[0])
    return rank, top, float(scores[true_key]), scores


def main():
    ap = argparse.ArgumentParser(description='AES-step 16-byte CPA summary')
    ap.add_argument('--npz', required=True)
    ap.add_argument('--n-traces', type=int, default=10000)
    ap.add_argument('--win-start', type=int, default=50)
    ap.add_argument('--win-end', type=int, default=2000)
    ap.add_argument('--out-prefix', default='aes_step_16b')
    args = ap.parse_args()

    d = np.load(args.npz)
    tr = d['traces'][:args.n_traces, args.win_start:args.win_end]
    pt = d['plaintexts'][:args.n_traces, :16].astype(np.uint8)
    key = d['key'][:16].astype(np.uint8)
    tr = preprocess(tr)

    ranks = []
    pois = []
    tops = []
    true_scores = []

    for b in range(16):
      corr = cpa_corr_curve(tr, pt[:, b], int(key[b]))
      poi = int(np.argmax(np.abs(corr)))
      rank, top, ts, _ = rank_and_scores_at_poi(tr[:, poi], pt[:, b], int(key[b]))
      ranks.append(rank)
      pois.append(args.win_start + poi)
      tops.append(top)
      true_scores.append(ts)
      print(f'byte{b:02d}: true=0x{int(key[b]):02x} rank={rank:3d} top=0x{top:02x} score_true={ts:.6f} poi={args.win_start+poi}')

    # Figure summary: rank per byte + true-score per byte
    fig, ax = plt.subplots(2, 1, figsize=(12, 8), sharex=True)
    x = np.arange(16)

    ax[0].bar(x, ranks)
    ax[0].set_yscale('log')
    ax[0].set_ylabel('Rank(true key)')
    ax[0].set_title('AES-step 16-byte CPA summary')
    ax[0].grid(True, which='both', ls='--', lw=0.5)

    ax[1].bar(x, true_scores, color='tab:orange')
    ax[1].set_ylabel('|corr|(true key @ POI)')
    ax[1].set_xlabel('byte index')

    for b in range(16):
        ax[0].annotate(f'{ranks[b]}', (b, ranks[b]), textcoords='offset points', xytext=(0, 4), ha='center', fontsize=8)

    plt.tight_layout()
    fig_path = f'{args.out_prefix}_analysis.png'
    fig.savefig(fig_path, dpi=140)

    # JSON summary
    js = {
      'npz': args.npz,
      'n_traces': int(args.n_traces),
      'window': [int(args.win_start), int(args.win_end)],
      'bytes': [
        {
          'byte': int(b),
          'true_key': int(key[b]),
          'rank': int(ranks[b]),
          'top_key': int(tops[b]),
          'poi': int(pois[b]),
          'true_score': float(true_scores[b]),
        } for b in range(16)
      ]
    }
    json_path = f'{args.out_prefix}_summary.json'
    with open(json_path, 'w') as f:
      json.dump(js, f, indent=2)

    rank1 = sum(1 for r in ranks if r == 1)
    rank5 = sum(1 for r in ranks if r <= 5)
    print(f'\nrank<=1: {rank1}/16 | rank<=5: {rank5}/16')
    print(f'saved: {fig_path}')
    print(f'saved: {json_path}')


if __name__ == '__main__':
    main()
