...
int main(void)
{
  ...
  uint8_t pt[16] = {0};
  uint8_t ct[16] = {0};
  uint32_t cnt_rx_cmd_err = 0;
  uint32_t cnt_bad_cmd = 0;
  uint32_t cnt_rx_pt_err = 0;
  uint32_t cnt_tx_ok = 0;
  uint32_t cnt_tx_err = 0;
  char dbg[96];
  ...
  while (1)
  {
    if (uart_recv_bytes(&cmd, 1) != HAL_OK) {
      cnt_rx_cmd_err++;
      uart_recover();
      if ((cnt_rx_cmd_err % 50U) == 0U) {
        snprintf(dbg, sizeof(dbg), "DBG RX1 ERR=%lu\r\n", (unsigned long)cnt_rx_cmd_err);
        uart_print_line(dbg);
      }
      continue;
    }
    if (cmd != 'P') {
      cnt_bad_cmd++;
      if (cnt_bad_cmd <= 10U || (cnt_bad_cmd % 100U) == 0U) {
        snprintf(dbg, sizeof(dbg), "DBG BAD CMD=0x%02X CNT=%lu\r\n", cmd, (unsigned long)cnt_bad_cmd);
        uart_print_line(dbg);
      }
      continue;
    }
    if (uart_recv_bytes(pt, sizeof(pt)) != HAL_OK) {
      cnt_rx_pt_err++;
      uart_recover();
      snprintf(dbg, sizeof(dbg), "DBG RX16 ERR=%lu\r\n", (unsigned long)cnt_rx_pt_err);
      uart_print_line(dbg);
      continue;
    }
    ...
    if (HAL_UART_Transmit(&huart2, resp, sizeof(resp), HAL_MAX_DELAY) != HAL_OK) {
      cnt_tx_err++;
      uart_recover();
      snprintf(dbg, sizeof(dbg), "DBG TX ERR=%lu\r\n", (unsigned long)cnt_tx_err);
      uart_print_line(dbg);
      continue;
    }
    cnt_tx_ok++;
    if ((cnt_tx_ok % 200U) == 0U) {
      snprintf(
        dbg,
        sizeof(dbg),
        "DBG OK=%lu RX1E=%lu BAD=%lu RX16E=%lu TXE=%lu\r\n",
        (unsigned long)cnt_tx_ok,
        (unsigned long)cnt_rx_cmd_err,
        (unsigned long)cnt_bad_cmd,
        (unsigned long)cnt_rx_pt_err,
        (unsigned long)cnt_tx_err
      );
      uart_print_line(dbg);
