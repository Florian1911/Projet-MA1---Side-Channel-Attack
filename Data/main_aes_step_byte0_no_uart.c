...
/**
  ******************************************************************************
  * @file           : main_aes_step_byte0_no_uart.c
  * @brief          : Pas intermediaire vers AES (byte 0 uniquement, no-UART).
  *
  *                   Sous trigger:
  *                     v = SBOX(PT[0] xor KEY[0])
  *                     puis transitions 0 <-> v repetees.
  *
  *                   Objectif: garder une operation AES-like (SBOX) mais avec
  *                   une fuite simplifiee et stable pour CPA sur byte 0.
  ******************************************************************************
  */
...
int main(void)
{
  ...
  for (uint32_t i = 0; i < N_PLAINTEXTS; i++) {
    ...
    const uint8_t v = SBOX[(uint8_t)(p ^ KEY_128[TARGET_BYTE])];
