#!/usr/bin/env python3
import argparse
import json
from pathlib import Path

import numpy as np

SBOX = np.array([
    
], dtype=np.uint8)
HW = np.array([bin(i).count('1') for i in range(256)], dtype=np.float64)


def preprocess(tr):
    t = tr.astype(np.float64, copy=False)
    t = t - t.mean(axis=1, keepdims=True)
    x = np.linspace(-1.0, 1.0, t.shape[1], dtype=np.float64)
    t = t - np.outer((t @ x) / np.dot(x, x), x)
    return t


def cpa_corr_curve(tr, pbyte, key):
    tc = tr - tr.mean(axis=0, keepdims=True)
    tstd = tr.std(axis=0, ddof=1) + 1e-15
    h = HW[SBOX[np.bitwise_xor(pbyte, key)]]
    hc = h - h.mean()
    hstd = h.std(ddof=1) + 1e-15
    return (hc @ tc) / ((tr.shape[0] - 1) * hstd * tstd)


def rank_and_scores_at_poi(y, pbyte, true_key):
    yc = y - y.mean()
    ystd = y.std(ddof=1) + 1e-15
    scores = np.zeros(256, dtype=np.float64)
    for k in range(256):
        h = HW[SBOX[np.bitwise_xor(pbyte, k)]]
        hc = h - h.mean()
        hstd = h.std(ddof=1) + 1e-15
        scores[k] = abs((hc @ yc) / ((len(y) - 1) * hstd * ystd))
    order = np.argsort(scores)[::-1]
    rank = int(np.where(order == true_key)[0][0]) + 1
    top = int(order[0])
    return rank, top


def eval_known_poi(tr, pt, key):
    ranks = []
    for b in range(16):
        corr = cpa_corr_curve(tr, pt[:, b], int(key[b]))
        poi = int(np.argmax(np.abs(corr)))
        rank, _ = rank_and_scores_at_poi(tr[:, poi], pt[:, b], int(key[b]))
        ranks.append(rank)
    return {
        "rank1_count": int(sum(r == 1 for r in ranks)),
        "rank5_count": int(sum(r <= 5 for r in ranks)),
        "mean_rank": float(np.mean(ranks)),
    }


def main():
    ap = argparse.ArgumentParser()
    ap.add_argument("--npz", default="dataset_aes_sca_highside.npz")
    ap.add_argument("--out", default="highside_knownpoi_sweep_summary.json")
    args = ap.parse_args()

    d = np.load(args.npz)
    src_map = {
        "traces": d["traces"],
        "traces_a": d["traces_a"],
        "traces_b": d["traces_b"],
    }
    pt = d["plaintexts"][:, :16].astype(np.uint8)
    key = d["key"][:16].astype(np.uint8)

    rows = []
    for src_name, src in src_map.items():
        for n in [200, 300, 500, 700, 1000]:
            tr = preprocess(src[:n])
            r = eval_known_poi(tr, pt[:n], key)
            row = {"source": src_name, "n_traces": n, **r}
            rows.append(row)
            print(f"[{src_name:8s} n={n:4d}] rank1={r['rank1_count']:2d}/16 rank5={r['rank5_count']:2d}/16")

    best = max(rows, key=lambda x: (x["rank1_count"], x["rank5_count"], -x["mean_rank"]))
    out = {"npz": args.npz, "rows": rows, "best": best}
    Path(args.out).write_text(json.dumps(out, indent=2))
    print("best:", best)
    print("saved:", args.out)


if __name__ == "__main__":
    main()
