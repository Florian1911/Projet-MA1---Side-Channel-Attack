#!/usr/bin/env python3
import argparse
import numpy as np
import matplotlib.pyplot as plt

SBOX = np.array([
    
], dtype=np.uint8)
HW = np.array([bin(i).count('1') for i in range(256)], dtype=np.float64)
TRUE_KEY = np.array([0x2b, 0x7e, 0x15, 0x16], dtype=np.uint8)


def preprocess(tr):
    t = tr.astype(np.float64, copy=False)
    t = t - t.mean(axis=1, keepdims=True)
    x = np.linspace(-1.0, 1.0, t.shape[1], dtype=np.float64)
    t = t - np.outer((t @ x) / np.dot(x, x), x)
    return t


def cpa_corr_curve(tr, pbyte, key):
    tc = tr - tr.mean(axis=0, keepdims=True)
    tstd = tr.std(axis=0, ddof=1) + 1e-15
    h = HW[SBOX[np.bitwise_xor(pbyte, key)]]
    hc = h - h.mean()
    hstd = h.std(ddof=1) + 1e-15
    return (hc @ tc) / ((tr.shape[0] - 1) * hstd * tstd)


def rank_at_poi(y, pbyte, true_key):
    yc = y - y.mean()
    ystd = y.std(ddof=1) + 1e-15
    scores = np.zeros(256, dtype=np.float64)
    for k in range(256):
        h = HW[SBOX[np.bitwise_xor(pbyte, k)]]
        hc = h - h.mean()
        hstd = h.std(ddof=1) + 1e-15
        scores[k] = abs((hc @ yc) / ((len(y) - 1) * hstd * ystd))
    order = np.argsort(scores)[::-1]
    return int(np.where(order == true_key)[0][0]) + 1


def main():
    ap = argparse.ArgumentParser(description='Convergence ranks for AES-step 4-byte target')
    ap.add_argument('--npz', required=True)
    ap.add_argument('--win-start', type=int, default=50)
    ap.add_argument('--win-end', type=int, default=1000)
    ap.add_argument('--n-list', default='500,1000,1500,2000,3000,4000,5000')
    ap.add_argument('--out', default='aes_step_4bytes_convergence.png')
    args = ap.parse_args()

    d = np.load(args.npz)
    tr = preprocess(d['traces'][:, args.win_start:args.win_end])
    pt = d['plaintexts'][:, :4].astype(np.uint8)

    n_list = [int(x) for x in args.n_list.split(',') if x.strip()]
    n_list = [n for n in n_list if n <= tr.shape[0]]

    pois = []
    for b in range(4):
        corr = cpa_corr_curve(tr, pt[:, b], int(TRUE_KEY[b]))
        pois.append(int(np.argmax(np.abs(corr))))

    ranks = {b: [] for b in range(4)}
    for n in n_list:
        for b in range(4):
            r = rank_at_poi(tr[:n, pois[b]], pt[:n, b], int(TRUE_KEY[b]))
            ranks[b].append(r)

    fig, ax = plt.subplots(figsize=(10, 6))
    colors = ['tab:blue', 'tab:orange', 'tab:green', 'tab:red']
    for b in range(4):
        ax.plot(n_list, ranks[b], marker='o', color=colors[b], label=f'byte {b} (poi={args.win_start+pois[b]})')

    ax.set_yscale('log')
    ax.set_xlabel('Number of traces')
    ax.set_ylabel('Rank(true key)')
    ax.set_title('AES-step 4-byte convergence')
    ax.grid(True, which='both', ls='--', lw=0.5)
    ax.legend()
    plt.tight_layout()
    fig.savefig(args.out, dpi=140)

    for b in range(4):
        print(f'byte{b} poi={args.win_start+pois[b]} ranks={ranks[b]}')
    print(f'saved: {args.out}')


if __name__ == '__main__':
    main()
